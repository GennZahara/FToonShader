Shader "Custom/Feel_Hair"
{
    Properties
    {
        ...
        [Toggle] _UseFloor   ("Use Floor Quantization", Float) = 0
        _CelSteps               ("Cel Steps", Range(2, 8)) = 2

        // 1st Shadow
        [Toggle(_USE_SHADOW_1ST)] _UseShadow1st ("Use 1st Shadow", Float) = 1
        ...
        // Hair Highlight
        [Toggle(_USE_HAIR_HIGHLIGHT)] _UseHairHighlight ("Use Hair Highlight",      Float) = 0
        [Toggle] _UseVirtualStrand ("Use Virtual Strand",      Float) = 0
        [Toggle] _UseBitangent     ("Use Bitangent (V dir)",   Float) = 0
        [Toggle] _UseKayToon       ("Use Kay Toon Step",       Float) = 0
        _HairToonThreshold      ("Toon Threshold",  Range(0, 1))   = 0.5
        _HairToonSmoothness     ("Toon Smoothness", Range(0, 0.5)) = 0.05

        _HairSpecColor1         ("Primary Spec Color",   Color)         = (1, 1, 1, 1)
        _HairShift1             ("Primary Shift",        Range(-1, 1))  = 0.0
        _HairExp1               ("Primary Exponent",     Range(1, 200)) = 50.0
        _HairInt1               ("Primary Intensity",    Range(0, 2))   = 0.5

        _HairSpecColor2         ("Secondary Spec Color", Color)         = (0.9, 0.85, 1, 1)
        _HairShift2             ("Secondary Shift",      Range(-1, 1))  = 0.3
        _HairExp2               ("Secondary Exponent",   Range(1, 200)) = 25.0
        _HairInt2               ("Secondary Intensity",  Range(0, 1))   = 0.2
        ...
        // Light Map (R:ShadowThreshold / G:SpecMask / B:RimMask)
        [Toggle] _UseLightMap ("Use Light Map", Float) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // shader_feature 미사용 — 토글 시 프레임 히치 없음
            ...
            #pragma fragment FeelHairFragment

            #include "Feel_Vertex.hlsl"
            #include "Feel_HairFragment.hlsl"
        ...
        Pass
        {
            ...
            HairShadowVaryings HairShadowVertex(HairShadowAttributes input)
            {
                HairShadowVaryings output;
            ...
            half4 HairShadowFragment(HairShadowVaryings input) : SV_Target
            {
                UNITY_SETUP_INSTANCE_ID(input);
    ...
    CustomEditor "FeelToon.ShaderInspector.FeelHairShaderGUI"
}
